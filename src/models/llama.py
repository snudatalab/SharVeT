...
from transformers.models.llama.modeling_llama import (LlamaAttention, LlamaDecoderLayer,
                                                      ...
                                                      LlamaForCausalLM, create_causal_mask)
...
class ShareLlamaAttention(LlamaAttention):
    def __init__(self, config, layer_idx, layer_to_group, k_basis, q_basis, v_basis, o_basis):
        ...
        self.q_proj = Coefficient(config.num_attention_heads * self.head_dim, q_num_basis)
        ...
        self.k_proj = Coefficient(config.num_key_value_heads * self.head_dim, k_num_basis)
        ...
        self.v_proj = Coefficient(config.num_key_value_heads * self.head_dim, v_num_basis)
        ...
        self.o_proj = Coefficient(config.hidden_size, o_num_basis)
...
class ShareLlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config, layer_idx, layer_to_group, k_basis, q_basis, v_basis, o_basis, up_basis, gate_basis, down_basis):
        ...
        self.self_attn = ShareLlamaAttention(config, layer_idx, layer_to_group,
                                                 k_basis[str(layer_idx)],
                                                 q_basis[str(layer_idx)],
                                                 v_basis[str(layer_idx)],
                                                 o_basis[str(layer_idx)])
        self.mlp = ShareLlamaMLP(config, layer_idx, layer_to_group, up_basis[str(layer_idx)],
    ...
    def forward(
        ...
        Returns:
            Outputs consistent with the base model.
        """
        """
        Args:
...
class ShareLlamaModel(LlamaModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [ShareLlamaDecoderLayer(config, layer_idx, self.layer_to_group, self.k_basis, self.q_basis, self.v_basis, self.o_basis, self.up_basis, self.gate_basis, self.down_basis) for layer_idx in range(config.num_hidden_layers)]
        )
        # self.post_init()

    def freeze_basis(self):
        """
        Freeze parameters in all basis collections if present.
    ...
    def forward(
        ...
        """
        Forward pass through decoder layers using shared basis modules.
